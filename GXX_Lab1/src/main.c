...
float am_times[3] = {0.00133383, 0.00100133, 0.00073067};
float sd_times[3] = {0.00292433, 0.00150842, 0.00053283};

float result_array[1000];
float32_t dsp_array[1000];
float asm_array[1000];
...
int main() {
	...
	/******Pure C Element-wise Array Multiplication********/
	for(i=0; i<1000; i++){
		result_array[i] = f1000_array[i]*f1000_array[i];
	}
	/******************************************************/
	
	/*****Assembly Element-wise Array Multiplication*******/
	asm_am(f1000_array, f1000_array, asm_array, 1000);
	/******************************************************/
	

	/***CMSIS-DSP Element-wise Array Multiplication********/
	arm_mult_f32(f1000_array, f1000_array, dsp_array, 1000);
	/******************************************************/
	
	printf("Pure C   : Array Multiplication = \n");
	printArray(1000, result_array);
	printf("\n");
	printf("Time: %f\n",(am_times[0]));
	printf("Assembly   : Array Multiplication = \n");
	printArray(1000, asm_array);
	printf("\n");
	printf("Time: %f; %f%% faster than pure C\n", (am_times[1]), (((am_times[0]-am_times[1])/am_times[0])*100));
	printf("CMSIS-DSP   : Array Multiplication = \n");
	printArray(1000, dsp_array);
	printf("\n");
	printf("Time: %f; %f%% faster than pure C and %f%% faster than Assembly\n", (am_times[2]), (((am_times[0]-am_times[2])/am_times[0])*100), (((am_times[1]-am_times[2])/am_times[1])*100));
	
	
	
	/*******Pure C Standard Deviation*******************/
	...
	sum = 0;
	for(i=0; i<1000; i++){
		sum+= square(f1000_array[i] - mean);
	}
	sd = sqrtf(sum/999);
	/**************************************************/
	
	/***********Assembly Standard Deviation***********/
	asm_sd(f1000_array, 1000, &asm_std);
	/**************************************************/
	
	/***********CMSIS-DSP Standard Deviation***********/
	arm_std_f32(f1000_array, 1000, &dsp_sd);
	/**************************************************/
	
	printf("Pure C   : Standard Deviation = %f\n", sd);
	printf("Time: %f\n",(sd_times[0]));
	printf("Assembly : Standard Deviation = %f\n", asm_std);
	printf("Time: %f; %f%% faster than pure C\n", (sd_times[1]), (((sd_times[0]-sd_times[1])/sd_times[0])*100));
	printf("CMSIS-DSP   : Standard Deviation = %f\n", dsp_sd);
	printf("Time: %f; %f%% faster than pure C and %f%% faster than Assembly\n", (sd_times[2]), (((sd_times[0]-sd_times[2])/sd_times[0])*100), (((sd_times[1]-sd_times[2])/sd_times[1])*100));
